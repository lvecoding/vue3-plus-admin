!(function () {
  ...
  const i = (e, t) =>
    ...
    m = [
      ...
      { name: 'Firefox', versionRegexes: [/.*?firefox\/\ ?([0-9]+)\.([0-9]+).*/], search: h('firefox') },
      {
        name: 'Safari',
    ...
    v = [
      { name: 'Windows', search: h('win'), versionRegexes: [/.*?windows\ nt\ ?([0-9]+)\.([0-9]+).*/] },
      {
        name: 'iOS',
      ...
      { name: 'Android', search: h('android'), versionRegexes: [/.*?android\ ?([0-9]+)\.([0-9]+).*/] },
      { name: 'macOS', search: h('mac os x'), versionRegexes: [/.*?mac\ os\ x\ ?([0-9]+)_([0-9]+).*/] },
    ...
    V = e => {
      const t = (e => {
        ...
        M.each(e.contentCSS, t => {
          r += '<link type="text/css" rel="stylesheet" href="' + n(e.documentBaseURI.toAbsolute(t)) + '"' + i + '>'
        }),
      ...
      e.windowManager
        .open({
          title: 'Preview',
          size: 'large',
          body: { type: 'panel', items: [{ name: 'preview', type: 'iframe', sandboxed: !0, transparent: !1 }] },
  ...
  e.add('preview', e => {
    ;(e => {
      ...
      (e => {
        ...
        e.ui.registry.addButton('preview', { icon: 'preview', tooltip: 'Preview', onAction: t, context: 'any' }),
          e.ui.registry.addMenuItem('preview', { icon: 'preview', text: 'Preview', onAction: t, context: 'any' })
      })(e)
  })
})()
